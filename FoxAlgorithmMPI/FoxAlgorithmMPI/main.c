...
{
    int Buff[10000];
    //int *Buff = malloc(BlockSize * Size * sizeof(MPI_INT));// буфер для рассылки блоков
    int i;
    if (GridCoords[1] == 0) 
...
int main (int argc, char * argv[] ) {
    ...
    if (ProcNum != GridSize*GridSize) {
        ...
        pAblock = malloc(BlockSize * BlockSize * sizeof(int));
        pBblock = malloc(BlockSize * BlockSize * sizeof(int));
        pCblock = malloc(BlockSize * BlockSize * sizeof(int));
        pMatrixAblock = malloc(BlockSize * BlockSize * sizeof(int));
        for (i = 0; i < BlockSize*BlockSize; ++i) {
            pCblock[i] = 0;
        }
        if (ProcRank == 0) {
            pAMatrix = malloc(Size * Size * sizeof(int));
            pBMatrix = malloc(Size * Size * sizeof(int));
            pCMatrix = malloc(Size * Size * sizeof(int));
